...
@section('title', 'News - '.$brand['acronym'])
...
@section('content')
    ...
    <section @class(['relative overflow-hidden bg-white flex items-center justify-center text-center px-6', 'min-h-screen' => $hasHero, 'py-16 md:py-24' => ! $hasHero])>
        ...
        @endif
        <h1 class="relative z-10 text-3xl md:text-4xl font-medium {{ $hasHero ? 'text-white' : 'text-[#111111]' }}">News &amp; Press</h1>
    </section>
    ...
    <section class="py-16 md:py-24 bg-white">
        <div class="max-w-6xl mx-auto px-6 lg:px-8">
            ...
            @if ($articles->isEmpty())
                <p class="text-center text-gray-500 py-16">No news yet.</p>
            @else
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach ($articles as $article)
                        <a href="{{ route('festival.news.show', $article->slug) }}" class="rounded-2xl border border-black/10 overflow-hidden hover:-translate-y-0.5 transition duration-300 bg-white">
